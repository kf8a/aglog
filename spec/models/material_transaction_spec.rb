  ...
  context 'A material transaction ' do
    ...
    context '180 lb of 46-0-0 ' do
      before(:each) do
        ...
        @material = FactoryBot.build(:material, name: 'n_content_material', n_content: 46)
        @unit = FactoryBot.build(:unit, conversion_factor: 453.592)
    ...
    context '14 lb/A of P2O5 ' do
      before(:each) do
        ...
        @material = FactoryBot.build(:material, name: 'p_content_material', p_content: 43.64)
        @unit = FactoryBot.build(:unit, conversion_factor: 453.592)
        @transaction.material = @material
        @transaction.unit = @unit
      end
    ...
    context '120 lb of K2O/A ' do
      before(:each) do
        ...
        @material = FactoryBot.build(:material, name: 'k_content_material', k_content: 49.8)
        @unit = FactoryBot.build(:unit, conversion_factor: 453.592)
        @transaction.material = @material
        @transaction.unit = @unit
      end
    ...
    context 'the transaction involves seeds ' do
      it 'gets the right rate when provided with seeds per acre' do
        @transaction.rate = 30_000
        material = FactoryBot.build(:material, name: 'seed')
        unit = FactoryBot.build(:unit, name: 'seeds')
        @transaction.material = material
        @transaction.unit = unit
    ...
    context 'The transaction has a unit. ' do
      before(:each) do
        @transaction.unit = FactoryBot.build(:unit, name: 'barrel')
        @transaction.material = FactoryBot.build(:material, name: 'Beets')
      end
    ...
    context 'The transaction does not have a unit. ' do
      before(:each) do
        ...
        @transaction.material = FactoryBot.build(:material, name: 'Carrots')
      end
